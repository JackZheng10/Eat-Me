...
class Session extends Component {
  ...
  fetchRestaurants = async () =>{
    ...
    for(fetchIndex = this.state.fetchIndex; fetchIndex < this.state.fetchIndex + 5; fetchIndex++){
  ...
  onSwiped = (index) => { 
    ...
    const needMoreRestaurants = this.state.fetchIndex - this.state.restaurantIndex < 3;
  ...
  renderRestaurant = (Restaurant, index) => {
    return(
      <RestaurantCard restaurant={this.state.currentRestaurant} images={this.state.currentRestaurant.photos} />
    );
  };
